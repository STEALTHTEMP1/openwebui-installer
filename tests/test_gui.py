...
def test_start_installation(window):
    ...
    with patch("openwebui_installer.gui.InstallerThread") as mock_thread_class, patch.object(
        window.install_button, "setEnabled"
    ) as mock_set_install_enabled, patch.object(
        window.uninstall_button, "setEnabled"
    ) as mock_set_uninstall_enabled, patch.object(
...
def test_handle_success(mock_msg_box, window):
    ...
    with patch.object(window, "update_status") as mock_update, patch.object(
        window.install_button, "setEnabled"
    ) as mock_set_enabled, patch.object(
        window.uninstall_button, "setEnabled"
    ) as mock_uninstall_set_enabled, patch.object(
